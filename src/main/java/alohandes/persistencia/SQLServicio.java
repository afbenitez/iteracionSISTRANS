...
public class SQLServicio {
	...
	public long adicionarServicio (PersistenceManager pm,long servicioId,String id , String nombre  ) 
	{
        Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaServicio()  + "( servicioId, id ,  nombre) values (?, ?, ?)");
        q.setParameters(servicioId, id ,  nombre);
        return (long) q.executeUnique();
	}
	public long eliminarServicioPorId (PersistenceManager pm, long id)
	{
        Query q = pm.newQuery(SQL, "DELETE FROM " + pp.darTablaServicio() + " WHERE ServicioID = ?");
        q.setParameters(id);
        return (long) q.executeUnique();
	}
	/**
	...
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaServicio () + " WHERE ServicioID = ?");
